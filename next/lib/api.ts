...
const API_BASE_URLS = {
  dev: '/anon-dev-server',
  prod: process.env.NEXT_PUBLIC_API_BASE_URL || 'http://anon.localhost:8080',
} as const

const DEFAULT_API_BASE_URL =
  process.env.NODE_ENV === 'development' ? API_BASE_URLS.dev : API_BASE_URLS.prod
...
async function request<T = any>(
  ...
  try {
    ...
    if (isAuthError) {
      // 尝试刷新 token
      if (retryOnAuth && typeof window !== 'undefined' && (await refreshToken(baseUrl))) {
    ...
    // 保存返回的 token
    if (typeof window !== 'undefined' && data.data && typeof data.data === 'object' && 'token' in data.data) {
    ...
    if (error instanceof Error) {
      ...
      if (isAuthError && retryOnAuth && typeof window !== 'undefined' && (await refreshToken(baseUrl))) {
        return request<T>(endpoint, options, false)
      }
